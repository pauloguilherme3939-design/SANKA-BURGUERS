...
function applyStatusEvents(order, events) {
  ...
  result.archived = Boolean(result.archived);
  result.archivedAt = result.archivedAt || null;
  result.administrativeHistory = Array.isArray(result.administrativeHistory)
    ? result.administrativeHistory
    : [];
  for (const event of [...events].sort((a, b) => a.ts.localeCompare(b.ts))) {
    if (event.kind === 'archived') {
      result.archived = event.archived !== false;
      result.archivedAt = result.archived ? event.ts : null;
      result.administrativeHistory.push({
        kind: result.archived ? 'archived' : 'restored',
        ts: event.ts,
      });
      continue;
    }
...
class FileOrderStore {
  ...
  appendAdministrativeEvent(id, event) {
    return this.appendStatus(id, event);
  }
}
