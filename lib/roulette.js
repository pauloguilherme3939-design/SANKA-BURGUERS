...
function normalizePhone(value) {
  try {
    ...
    if (error instanceof PhoneValidationError) {
      throw new RouletteError('Informe um telefone brasileiro válido.', 400, 'INVALID_PHONE');
    }
    throw error;
  }
}
...
function createRouletteService({
  ...
  return {
    ...
    async cancelByOrderId(orderId, reason = 'pedido_cancelado') {
      requireSecret(safeSecret);
      const normalizedOrderId = normalizeOrderId(orderId);
      const orderDay = orderDayFromId(normalizedOrderId);
      const records = await store.findByOrderId(orderDay, normalizedOrderId);
      const cancelled = [];
      for (const record of records) {
        ...
        const event = {
          ...
          reason: String(reason || 'pedido_cancelado').slice(0, 80),
        };
        await store.markCancelled(record, event);
