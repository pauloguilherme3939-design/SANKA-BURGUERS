...
class FileRouletteStore {
  ...
  async findByOrderId(day, orderId) {
    const records = await this.listDay(day);
    return records.filter(record => record?.orderId === orderId || record?.cancelledOrderId === orderId);
  }
...
function createDefaultRouletteStore(env = process.env) {
  ...
  const rouletteEnabled = enabled(env.ROULETTE_ENABLED) && enabled(env.ROULETTE_LEGAL_APPROVED);

  // Em ambiente publicado, a roleta desligada recebe um store inerte: nenhuma
  // conexão é criada e nenhuma leitura ou gravação externa pode acontecer.
  if (!rouletteEnabled && (env.VERCEL || env.VERCEL_ENV)) {
    const unavailable = async () => {
      throw new StorageError('A Roleta Sanka está desativada.');
    };
    return {
      getByPhone: unavailable,
      getByCode: unavailable,
      create: unavailable,
      claimUse: unavailable,
      markExpired: unavailable,
      markCancelled: unavailable,
    };
  }
  if (env.VERCEL || env.VERCEL_ENV) {
    throw new StorageError('A Roleta Sanka não possui armazenamento homologado para produção.');
  }
