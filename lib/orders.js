...
const { normalizeBrazilianPhone, PhoneValidationError } = require('./br-phone.js');
...
function validateAndPricePayload(payload, { now = new Date(), idFactory = makeOrderId } = {}) {
  ...
  let phone = '';
  try {
    phone = normalizeBrazilianPhone(body.customer?.phone);
  } catch (error) {
    if (!(error instanceof PhoneValidationError)) throw error;
  ...
  return {
    ...
    async updateStatus(id, requestedStatus) {
      ...
      if (current.status === CANCELLED_STATUS) {
        throw new OrderError('Pedido cancelado não pode avançar de status.', 409, 'ORDER_CANCELLED');
      }
    ...
    async cancel(id, cancellation = {}) {
      ...
      const current = await store.get(normalizedId);
      if (!current) throw new OrderError('Pedido não encontrado.', 404, 'ORDER_NOT_FOUND');
      if (current.status === CANCELLED_STATUS) return current;
      if (current.status === 'entregue') {
        throw new OrderError('Pedido entregue não pode ser cancelado.', 409, 'ORDER_ALREADY_DELIVERED');
      }
      const event = {
        kind: 'cancelled',
        status: CANCELLED_STATUS,
        ts: now().toISOString(),
        reasonCode: cleanText(cancellation.reasonCode || 'administrative', 40),
        reason: cleanText(cancellation.reason, 160),
      };
      await store.appendCancellation(normalizedId, event);
      return {
        ...current,
        status: CANCELLED_STATUS,
        updatedAt: event.ts,
        ...
        history: [...current.history, { status: CANCELLED_STATUS, ts: event.ts }],
      };
    },
    ...
    async archive(id) {
      ...
      const current = await store.get(normalizedId);
      if (!current) throw new OrderError('Pedido não encontrado.', 404, 'ORDER_NOT_FOUND');
      if (!['entregue', CANCELLED_STATUS].includes(current.status)) {
        throw new OrderError('Finalize ou cancele o pedido antes de arquivar.', 409, 'ORDER_NOT_TERMINAL');
      }
      if (current.archived) return current;
      const event = { kind: 'archived', archived: true, ts: now().toISOString() };
      await store.appendAdministrativeEvent(normalizedId, event);
      return {
        ...current,
        archived: true,
        archivedAt: event.ts,
        administrativeHistory: [
          ...(current.administrativeHistory || []),
          { kind: 'archived', ts: event.ts },
        ],
      };
    },
    ...
    async restore(id) {
      ...
      if (!current) throw new OrderError('Pedido não encontrado.', 404, 'ORDER_NOT_FOUND');
      if (!current.archived) return current;
      const event = { kind: 'archived', archived: false, ts: now().toISOString() };
      await store.appendAdministrativeEvent(normalizedId, event);
      return {
        ...current,
        archived: false,
